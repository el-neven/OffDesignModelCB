...
class HP():
    ...
    def system(self, x):
        ...
        self.COP_HP = self.Q_dot_cd/self.W_dot_tot
        ...
        "For the graphs"
        print('res', self.res_1, self.res_2)
        print(self.EVAP.flag, self.COND.flag)
    ...
    def solve(self):
            ...
            while not stop and j < len(x_pp_guess):
                ...
                while not stop and i < len(x_pp_guess):
                    P_cd_guess = PropsSI('P', 'T', self.T_w_cd_su+self.glide_cd+x_pp_guess[j]+5, 'Q', 0, 'R1233ZDE')
                    P_ev_guess = PropsSI('P', 'T', self.T_w_ev_su-self.glide_ev-x_pp_guess[i]-5, 'Q', 0, 'R1233ZDE')
                    # rp_guess = P_cd_guess/P_ev_guess
                    try:
                        root(self.system, [P_cd_guess, P_ev_guess], tol=1e-5, method='lm')
            ...
            # P_cd_lb = PropsSI('P', 'T', self.T_w_cd_su+self.glide_cd+self.T_sc+2, 'Q', 0, self.wf)
            # P_ev_ub = PropsSI('P', 'T', self.T_w_ev_su-self.glide_ev-self.T_sc-2, 'Q', 0, self.wf)

            # rp_min = P_cd_lb/P_ev_ub
            # rp_max = 10
            # P_cd_ub = P_ev_ub*rp_max

            # P_cd_guess = P_cd_lb
            # rp_guess = rp_min

            # bounds = [(P_cd_lb, rp_min), (P_cd_ub, rp_max)]
            # # self.system([P_cd_guess, rp_guess])
            # least_squares(self.system, [P_cd_guess, rp_guess], bounds=bounds, xtol=1e-4)
...
if __name__ == '__main__':
    ...
    start_time = time.time()
    HP = HP()
    HP.inputs(6000, 293, 'R1233ZDE', 90+273.15, 14, 62+273.15, 14, 2)
    HP.set_parameters(**{
        'T_sh': 5,
        'T_sc' : 5,
    })
