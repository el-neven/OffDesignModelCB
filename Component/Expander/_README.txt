...
-Design parameters: AU_amb [W/K], AU_su_n [W/K], AU_ex_n [W/K], d_su1 [m], m_dot_n [kg/s], A_leak [m^2], W_dot_loss_0 [W], alpha [-], C_loss [Nm], T_amb [K]
...
4 different code:
	...
	- Expander_SE: Contains the Expander class
	- Calibration_expander: Contains the minimization code to determine the parameters based on the experimental data
	- Calibration_graphs: parity plots of the measured values vs the calculated ones
	- Expander_test: example of how to use the semi-empirical model with a plot of epsilon_vs_rp

2. Exeprimental datas

Nothing special to point out. Maybe a problem with the speed measurement. 150 experimental point were used to fit the model. The mass flow rate is very well fitted while the work at the shaft is not that good. 
